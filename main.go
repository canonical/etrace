...
type cmdRun struct {
	WindowName    string `short:"n" long:"window-name" description:"Window name to wait for" required:"yes"`
...
func (x *cmdRun) Execute(args []string) error {
	...
	cmd, err := strace.TraceExecCommand(straceLog, x.Args.Cmd...)
	if err != nil {
		return err
	}
	cmd.Stdin = os.Stdin
	...
	tryXToolClose := true
	tryWmctrl := false
	wids, err := xtool.waitForWindows(x.Args.Cmd[0])
	if err != nil {
